...
export default function MyItemsPage() {
  ...
  return (
    <Container className="space-y-8 pb-12 pt-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-brand-700">
            Lacak dan kelola barang yang telah Anda laporkan. Jika barang hilang Anda sudah kembali, tandai sebagai sudah ditemukan untuk memperbarui informasi di kampus.
          </p>
        </div>
      ...
      {myItems.length === 0 ? (
        /* Empty State Custom agar senada dengan UI baru */
        <div className="flex flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-brand-300 bg-canvas py-20 text-center">
          ...
          <h3 className="font-display text-xl text-brand-900">Belum ada item milik Anda</h3>
          <p className="mt-2 text-sm text-brand-500">Setelah Anda membuat laporan, item akan langsung muncul di halaman ini.</p>
        </div>
      ) : (
        /* List Barang Management View */
        <div className="flex flex-col overflow-hidden rounded-[2rem] border border-brand-100 bg-white shadow-soft">
          {myItems.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className={`flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:gap-6 sm:p-6 transition-colors hover:bg-brand-100/20 ${
                  index !== myItems.length - 1 ? 'border-b border-brand-100' : ''
                }`}
              >
                {/* Thumbnail */}
                <Link to={`/barang/${item.id}`} className="shrink-0 overflow-hidden rounded-2xl">
                  <img
                    src={item.imageUrl || 'https://via.placeholder.com/150?text=No+Image'}
                    alt={item.title}
                    className="h-24 w-24 object-cover sm:h-28 sm:w-28 transition-transform duration-300 hover:scale-110"
                  />
                </Link>
                ...
                <div className="flex flex-1 flex-col justify-center space-y-1.5">
                  ...
                  <Link to={`/barang/${item.id}`} className="group inline-block w-fit">
